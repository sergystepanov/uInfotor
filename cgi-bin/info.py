...
if r:
    ...
    print('<td style="width: 18%; vertical-align:middle"><p class="info" title="{} байт">Размер: <b>{}</b></p></td>'.format(sepp(r[4]),calc(r[4])))
    print('<td style="width: 10%; vertical-align:middle" align="right"><div class="folder" style="display:{}"><div class="files"><h2 style="margin-bottom:3px">Список файлов:</h2>'.format(exist_table()))
    print('''<table class="subtable">''')
    if len(fl)>0:
        offset=1
        
        if int(fl[0][2])==0:
            print('''<tr><td colspan=2 style="background-color:#bad5cc; padding-left: {}em"><img class="td-folder" src="../IMG/opened-folder.png"><h4 style="display:inline">{}</h4></td></tr>'''.format(offset,fl[0][1]))
            cat="f"
        else:
            print('''<tr><td style="padding-left: {}em"><img class="td-file" src="../IMG/file.png">{}</td><td align="right" style="padding-right: 1em;width: 7em;" title="{} байт">{}</td></tr>'''.format(offset+1,fl[0][1],sepp(fl[0][2]),calc(int(fl[0][2]))))
            cat="d"
            offset +=1
        for item in fl[1:]:
            if int(item[2])==0:
                if cat!="d": offset +=2
                if cat=="f": offset -=2
                print('''<tr><td colspan=2 style="background-color:#d1e6df; padding-left: {}em"><img class="td-folder" src="../IMG/opened-folder.png"><h4 style="display:inline">{}</h4></td></tr>'''.format(offset+1,item[1]))
                cat="d"
            else:
                print('''<tr><td style="padding-left: {}em"><img class="td-file" src="../IMG/file.png">{}</td><td align="right" style="padding-right: 1em; width: 7em;" title="{} байт">{}</td></tr>'''.format(offset+2,item[1],sepp(item[2]),calc(int(item[2]))))
                cat="f"
    print('''</table><br>''')
    print('</div></div>')
    print('<td style="width: 15%; vertical-align:middle" align="right"><p class="info"><b>{}</b> {}</p></td>'.format(r[5][:10],r[5][11:]))
